...
use llvm_sys::{LLVMTypeKind, LLVMValueKind, LLVMIntPredicate, LLVMRealPredicate};
use libc::{c_char, c_uint, c_double, c_ulonglong};
...
impl Context {
    ...
    pub fn struct_ty(&self, types: &mut Vec<LLVMTypeRef>, packed: bool) -> LLVMTypeRef {
        unsafe {
            LLVMStructTypeInContext(self.into(), types.as_mut_ptr(), types.len() as c_uint, llvm_bool!(packed))
        }
    }
...
pub fn function_ty(args: &mut Vec<LLVMTypeRef>, ret: LLVMTypeRef, variadic: bool) -> LLVMTypeRef {
    unsafe {
        LLVMFunctionType(ret, args.as_mut_ptr(), args.len() as c_uint, llvm_bool!(variadic))
    }
}
...
pub fn const_int(ty: LLVMTypeRef, val: usize, sign_extend: bool) -> LLVMValueRef {
    unsafe {
        LLVMConstInt(ty, val as c_ulonglong, llvm_bool!(sign_extend))
    }
}
...
pub fn add_incoming(phi: LLVMValueRef, incoming: &mut Vec<(LLVMValueRef, LLVMBasicBlockRef)>) {
    unsafe {
        ...
        let mut blocks = incoming.iter().map(|x| x.1).collect::<Vec<LLVMBasicBlockRef>>();
        LLVMAddIncoming(phi, values.as_mut_ptr(), blocks.as_mut_ptr(), incoming.len() as c_uint)
    }
}
...
impl Builder {
    ...
    pub fn build_icmp(&mut self, op: LLVMIntPredicate, lhs: LLVMValueRef, rhs: LLVMValueRef) -> LLVMValueRef {
        unsafe { LLVMBuildICmp(self.into(), op, lhs, rhs, EMPTY_STR.as_ptr() as *const c_char) }
    }

    pub fn build_fcmp(&mut self, op: LLVMRealPredicate, lhs: LLVMValueRef, rhs: LLVMValueRef) -> LLVMValueRef {
        unsafe { LLVMBuildFCmp(self.into(), op, lhs, rhs, EMPTY_STR.as_ptr() as *const c_char) }
    }
    ...
    pub fn build_cond_br(&mut self, cond: LLVMValueRef, if_block: LLVMBasicBlockRef, else_block: LLVMBasicBlockRef) -> LLVMValueRef {
    ...
    pub fn build_gep(&mut self, ptr: LLVMValueRef, args: &mut Vec<LLVMValueRef>) -> LLVMValueRef {
        unsafe {
            LLVMBuildGEP(self.into(), ptr, args.as_mut_ptr(), args.len() as c_uint, EMPTY_STR.as_ptr() as *const c_char)
        }
    }

    pub fn build_gep_in_bounds(&mut self, ptr: LLVMValueRef, args: &mut Vec<LLVMValueRef>) -> LLVMValueRef {
        unsafe {
            LLVMBuildInBoundsGEP(self.into(), ptr, args.as_mut_ptr(), args.len() as c_uint, EMPTY_STR.as_ptr() as *const c_char)
        }
    }

    pub fn build_bit_cast(&mut self, val: LLVMValueRef, dest_ty: LLVMTypeRef) -> LLVMValueRef {
        unsafe { LLVMBuildBitCast(self.into(), val, dest_ty, EMPTY_STR.as_ptr() as *const c_char) }
    }

    pub fn build_ptr_to_int(&mut self, val: LLVMValueRef, dest_ty: LLVMTypeRef) -> LLVMValueRef {
        unsafe { LLVMBuildPtrToInt(self.into(), val, dest_ty, EMPTY_STR.as_ptr() as *const c_char) }
    }
    ...
    pub fn build_free(&mut self, val: LLVMValueRef) -> LLVMValueRef {
        unsafe {
            LLVMBuildFree(self.into(), val)
        }
    }

    pub fn build_call(&mut self, func: LLVMValueRef, args: &mut Vec<LLVMValueRef>) -> LLVMValueRef {
        unsafe {
            LLVMBuildCall(self.into(), func, args.as_mut_ptr(), args.len() as c_uint, EMPTY_STR.as_ptr() as *const c_char)
        }
    }
...
impl Module {
    ...
    pub fn add_function(&mut self, name: &str, args: &mut Vec<LLVMTypeRef>, ret: LLVMTypeRef, variadic: bool) -> LLVMValueRef {
        let c_name = CString::new(name).unwrap();
        let fn_name = c_name.to_bytes_with_nul().as_ptr() as *const c_char;
        let fn_ty = function_ty(args, ret, variadic);
        unsafe {
            LLVMAddFunction(self.into(), fn_name, fn_ty)
        }
    }
    ...
    pub fn emit_object_file(&self, target: &TargetMachine, filename: &str) -> Result<(), CString> {
        ...
        unsafe {
            ...
            let mut error = mem::uninitialized();
            if LLVMTargetMachineEmitToFile(target.into(), self.into(), c_filename, file_type, &mut error) == 1 {
                let message = CStr::from_ptr(error).to_owned();
                LLVMDisposeMessage(error);
                Err(message)
            } else {
